...
`DrawGui()` in `SceneMuntasir` calls helpers in order:
...
DrawGui()
    ...
    │     ├── Level timer             — top, pinned left of camera debug panel (cx-200, 20)
    │     └── Build version label     — bottom-right (x-10, y-10), both game and title scenes
...
Separate floating window pinned at `(cx - 200, 20)` pivot `(1, 0)`, sitting just left of the camera debug panel. Dark navy background, cyan border (via `PushStyleColor`). Displays:
- `L E V E L` header at body font (`AppFonts::body`)
- Large `MM:SS` digits using `PushFont(AppFonts::large)`: cyan minutes, pulsing white colon (`sinf(t * kPi)`), violet seconds

### Build version label

Transparent floating window anchored bottom-right `(x-10, y-10)` pivot `(1,1)` in both `SceneMuntasir` and `SceneTitle`. Reads values from `AppVersion` in `Version.h`:
```cpp
ImGui::TextDisabled("Alpha Engine  v%d.%d.%d  build %d",
    AppVersion::kMajor, AppVersion::kMinor, AppVersion::kPatch, AppVersion::kBuild);
```
To bump the version, edit only `Version.h`.

### Font system (`AppFonts`)

`AppFonts.h/.cpp` exposes four `ImFont*` pointers loaded once in `SceneManager::Initialize()` from `fonts/Exo2-Regular.ttf` (Exo 2, Google Fonts, OFL). Falls back to ImGui's built-in default if the file is missing.
...
**Rule:** always use `PushFont(ptr)` / `PopFont()` — never `SetWindowFontScale()`. `SetWindowFontScale` stretches the rasterized texture and causes blur. `PushFont(nullptr)` is safe and silently uses the current default.
